...
$(function () {
  ...
  function buildClones() {
    ...
    for (let i = clones - 1; i >= 0; i--) {
      const $s = $realSlides.eq(ORIGINAL_SLIDES - 1 - i).clone().addClass('clone');
      $track.prepend($s);
    }
  ...
  function goTo(i, withTransition = true) {
    if (!withTransition) $track.css('transition', 'none'); else $track.css('transition', 'transform 0.45s ease');
    $track.css('transform', 'translateX(' + (-i * slideWidth) + 'px)');
    index = i;
  }

  function next() { if (isTransitioning) return; isTransitioning = true; goTo(index + 1); }
  function prev() { if (isTransitioning) return; isTransitioning = true; goTo(index - 1); }
  ...
  function pointerDown(e) {
    startX = e.originalEvent.clientX || (e.originalEvent.touches && e.originalEvent.touches[0].clientX);
    startY = e.originalEvent.clientY || (e.originalEvent.touches && e.originalEvent.touches[0].clientY);
    dragging = true;
    stopAutoplay();
  ...
  function pointerMove(e) {
    if (!dragging) return;
    const x = e.originalEvent.clientX || (e.originalEvent.touches && e.originalEvent.touches[0].clientX);
    const y = e.originalEvent.clientY || (e.originalEvent.touches && e.originalEvent.touches[0].clientY);
    const dx = x - startX;
    const dy = y - startY;
  ...
  function pointerUp(e) {
    if (!dragging) return;
    dragging = false;
    const x = e.originalEvent.clientX || (e.originalEvent.changedTouches && e.originalEvent.changedTouches[0].clientX);
    const dx = x - startX;
